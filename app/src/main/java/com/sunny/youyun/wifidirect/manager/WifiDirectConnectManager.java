package com.sunny.youyun.wifidirect.manager;

import android.net.wifi.p2p.WifiP2pConfig;
import android.net.wifi.p2p.WifiP2pManager;

/**
 * Created by Sunny on 2017/7/31 0031.
 */

interface WifiDirectConnectManager {
    /**
     * This method create a p2p group with the current device as the group owner.(创建群组)
     *
     * @param createGroupListener for callbacks on success or failure. Can be null.
     */
    void createGroup(WifiP2pManager.ActionListener createGroupListener);

    /**
     * This method remove current group, if the current device is not a group owner, this method
     * will made the current connection disconnect.
     * （该方法用于移除群组，如果调用设备不是群主，则断开当前设备与群主的连接）
     *
     * @param removeGroupListener for callbacks on success or failure. Can be null.
     */
    void disConnect(WifiP2pManager.ActionListener removeGroupListener);

    /**
     * This method remove current group, if the current device is not a group owner, this method
     * will made the current connection disconnect.
     * （该方法用于移除群组，如果调用设备不是群主，则断开当前设备与群主的连接）
     * <p>
     * ps: User don't care about the result of this operate
     */
    void disConnect();


    /**
     * Cancel any ongoing p2p group negotiation(取消正在尝试的连接)
     *
     * @param actionListener for callbacks on success or failure. Can be null.
     */
    void cancelConnect(WifiP2pManager.ActionListener actionListener);

    /**
     * Initiate peer discovery. A discovery process involves scanning for available Wi-Fi peers
     * for the purpose of establishing a connection.
     * (开始搜索附近的设备)
     *
     * @param discoverListener for callbacks on success or failure. Can be null.
     */
    void discover(WifiP2pManager.ActionListener discoverListener);

    /**
     * Initiate peer discovery. A discovery process involves scanning for available Wi-Fi peers
     * for the purpose of establishing a connection.
     * 开始搜索附近的设备
     * <p>
     * ps: User don't care about the result of this operate
     */
    void discover();


    /**
     * Start a p2p connection to a device with the specified configuration.
     * 连接（用户自定义监听器）
     *
     * @param config   options as described in {@link WifiP2pConfig} class
     * @param listener for callbacks on success or failure. Can be null.
     *                 连接状态监听器（onSuccess只是代表该操作成功， 并不能说明连接成功，需要在receive那边判断）
     */
    void connect(WifiP2pConfig config, WifiP2pManager.ActionListener listener);

    /**
     * Start a p2p connection to a device with the specified configuration.
     * 连接（用户自定义监听器）
     * <p>
     * ps: User don't care about the result of this operate
     *
     * @param config options as described in {@link WifiP2pConfig} class
     */
    void connect(WifiP2pConfig config);


    void requestPeers(WifiP2pManager.PeerListListener peerListListener);

    void requestConnectionInfo(WifiP2pManager.ConnectionInfoListener connectionInfoListener);

    void requestGroupInfo(WifiP2pManager.GroupInfoListener groupInfoListener);

}
